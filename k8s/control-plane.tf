...
resource "null_resource" "save_kube_config" {
  ...
  provisioner "local-exec" {
    command = var.linux_overwrite_local_kube_config ? "cp .terraform/.kube/config-external ~/.kube/config" : "echo Kube config is available locally: .terraform/.kube/config-external"
...
resource "oci_network_load_balancer_backend_set" "control_plane_tcp_6443" {
  network_load_balancer_id = var.load_balancer_id
  name                     = "control_plane_tcp_6443"
  policy                   = "FIVE_TUPLE"
...
resource "oci_network_load_balancer_backend" "control_plane_tcp_6443" {
  backend_set_name         = oci_network_load_balancer_backend_set.control_plane_tcp_6443.name
  network_load_balancer_id = var.load_balancer_id
  name                     = "control_plane_tcp_6443"
  port                     = 6443
  ...
  depends_on = [
    oci_network_load_balancer_backend_set.control_plane_tcp_6443
  ]
}

resource "oci_network_load_balancer_listener" "control_plane_tcp_6443" {
  default_backend_set_name = oci_network_load_balancer_backend_set.control_plane_tcp_6443.name
  name                     = "control_plane_tcp_6443"
  network_load_balancer_id = var.load_balancer_id
  port                     = 6443
  protocol                 = "TCP"
  ...
  depends_on = [
    oci_network_load_balancer_backend_set.control_plane_tcp_6443,
    oci_network_load_balancer_backend.control_plane_tcp_6443
  ]
}
