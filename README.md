...
- [Configuração dos Gateways no ChirpStack](#configuração-dos-gateways-no-chirpstack)
  - [Plano de canais AU915 — Brasil](#plano-de-canais-au915--brasil)
...
- [Classes de dispositivos LoRaWAN — A, B e C](#classes-de-dispositivos-lorawan--a-b-e-c)
  - [Class A — Aloha](#class-a--aloha)
  - [Class B — Beacon](#class-b--beacon)
  - [Class C — Continuous](#class-c--continuous)
  - [Comparativo das três classes](#comparativo-das-três-classes)
  - [Bibliotecas e suporte por classe](#bibliotecas-e-suporte-por-classe)
- [Envio de Downlinks via MQTT](#envio-de-downlinks-via-mqtt)
  - [Instalação do cliente MQTT](#instalação-do-cliente-mqtt)
  - [Conceitos básicos — pub/sub e tópicos](#conceitos-básicos--pubsub-e-tópicos)
  - [Disparar downlink — ChirpStack v3 (sem TLS)](#disparar-downlink--chirpstack-v3-sem-tls)
  - [Disparar downlink — ChirpStack v4 (com TLS)](#disparar-downlink--chirpstack-v4-com-tls)
  - [Monitorar uplinks e eventos via MQTT](#monitorar-uplinks-e-eventos-via-mqtt)
...
- [Troubleshooting](#troubleshooting)
  - [❌ Downlinks não chegam ao dispositivo Class A](#-downlinks-não-chegam-ao-dispositivo-class-a)
  - [❌ Downlink descartado — payload excede o tamanho máximo](#-downlink-descartado--payload-excede-o-tamanho-máximo-downlink_payload_size)
  - [❌ Let's Encrypt — `429 rateLimited`: limite de 5 certificados por semana atingido](#-lets-encrypt--429-ratelimited-limite-de-5-certificados-por-semana-atingido)
  - [❌ GitHub Actions — `Input required and not supplied: token`](#-github-actions--input-required-and-not-supplied-token)
  - [❌ GitHub Actions — `refusing to allow a Personal Access Token to create or update workflow`](#-github-actions--refusing-to-allow-a-personal-access-token-to-create-or-update-workflow)
  - [❌ `409-NAMESPACE_CONFLICT` ao criar repositório no OCI Container Registry via Terraform](#-409-namespace_conflict-ao-criar-repositório-no-oci-container-registry-via-terraform)
  - [❌ Docker build — `npm error Missing: <pacote>@<versão> from lock file`](#-docker-build--npm-error-missing-pacoteverso-from-lock-file)
  - [❌ ABP — "Frame-counter reset or rollover detected" após reset do dispositivo](#-abp--frame-counter-reset-or-rollover-detected-após-reset-do-dispositivo)
  - [❌ Uplinks chegam ao gateway mas nunca aparecem nos Events do device (MIC inválido — MCCI LMiC v6)](#-uplinks-chegam-ao-gateway-mas-nunca-aparecem-nos-events-do-device-mic-inválido--mcci-lmic-v6)
...
enabled_uplink_channels = [8, 9, 10, 11, 12, 13, 14, 15, 65]  # Sub-banda 1
```
...
Cada versão do ChirpStack possui **8 Device Profiles** — 4 para ABP e 4 para OTAA — cobrindo as duas sub-bandas AU915 (0 e 1) e as duas revisões de parâmetros regionais (A e B).

---

### Plano de canais AU915 — Brasil

O **AU915** é o plano regional LoRaWAN para a faixa de 915–928 MHz, regulamentado pela ANATEL para uso no Brasil. Ele define 72 canais de uplink (dispositivo → gateway) e 8 canais de downlink (gateway → dispositivo), organizados da seguinte forma:

**Uplink — 72 canais no total:**

- **64 canais BW125** (largura de banda 125 kHz) — canais 0 a 63, espaçados 200 kHz, de 915.2 MHz a 927.8 MHz, usados com SF7 a SF12 (DR0–DR5).
- **8 canais BW500** (largura de banda 500 kHz) — canais 64 a 71, espaçados 1.6 MHz, de 915.9 MHz a 927.1 MHz, usados exclusivamente com SF8 (DR6) para JOIN requests em OTAA.

**Downlink — 8 canais BW500:**

Canais 0 a 7, de 923.3 MHz a 924.7 MHz, espaçados 600 kHz, usados pelo gateway para enviar respostas nas janelas RX1 e RX2.

#### Divisão em sub-bandas

Os 72 canais de uplink são agrupados em **8 sub-bandas** de 9 canais cada (8 BW125 + 1 BW500). Na prática, dispositivos e gateways operam em apenas uma sub-banda por vez — isso evita colisões e permite que múltiplas redes coexistam na mesma região:

| Sub-banda | Canais BW125 | Canal BW500 | Frequências BW125 (MHz) | Frequência BW500 (MHz) |
|---|---|---|---|---|
| 0 (`au915_0`) | 0 – 7 | 64 | 915.2 – 916.6 | 915.9 |
| **1 (`au915_1`)** | **8 – 15** | **65** | **916.8 – 918.2** | **917.5** |
| 2 (`au915_2`) | 16 – 23 | 66 | 918.4 – 919.8 | 919.1 |
| 3 (`au915_3`) | 24 – 31 | 67 | 920.0 – 921.4 | 920.7 |
| 4 (`au915_4`) | 32 – 39 | 68 | 921.6 – 923.0 | 922.3 |
| 5 (`au915_5`) | 40 – 47 | 69 | 923.2 – 924.6 | 923.9 |
| 6 (`au915_6`) | 48 – 55 | 70 | 924.8 – 926.2 | 925.5 |
| 7 (`au915_7`) | 56 – 63 | 71 | 926.4 – 927.8 | 927.1 |

#### Por que sub-banda 1 neste projeto

A comunidade brasileira adotou a **sub-banda 1** (`au915_1`, canais 8–15 + 65) como padrão de facto. Os gateways e dispositivos deste projeto operam exclusivamente nessa sub-banda, o que reflete diretamente em três lugares:

**1. ChirpStack v3 — ConfigMap do Network Server:**
```toml
[network_server.network_settings]
enabled_uplink_channels = [8, 9, 10, 11, 12, 13, 14, 15, 65]  # Sub-banda 1
```

**2. ChirpStack v4 — arquivo de região:**
```toml
# region_au915_1.toml
[[regions]]
id = "au915_1"
...
**3. Firmware do dispositivo (MCCI LMiC):**
```cpp
// Desabilita todas as sub-bandas e canais
for (u1_t b  =  0; b  <  8; ++b)   LMIC_disableSubBand(b);
for (u1_t ch =  0; ch < 72; ++ch)  LMIC_disableChannel(ch);
// Habilita apenas a sub-banda 1
for (u1_t ch =  8; ch <= 15; ++ch) LMIC_enableChannel(ch); // BW125
LMIC_enableChannel(65);                                      // BW500
```

> ⚠️ **Gateway e dispositivo devem estar na mesma sub-banda.** Se o gateway escuta a sub-banda 0 (canais 0–7 + 64) e o dispositivo transmite na sub-banda 1 (canais 8–15 + 65), os frames chegam ao gateway mas o ChirpStack não os associa a nenhum device — o sintoma é exatamente o log `None of the device-sessions for dev_addr resulted in valid MIC`. Confirme sempre a sub-banda nos três pontos acima.

#### Detalhamento da sub-banda 1 — canais e frequências

| Canal | Tipo | Frequência (MHz) | BW (kHz) | DR suportados | Uso |
|---|---|---|---|---|---|
| 8 | BW125 | 916.8 | 125 | DR0–DR5 (SF12–SF7) | Uplink de dados |
| 9 | BW125 | 917.0 | 125 | DR0–DR5 | Uplink de dados |
| 10 | BW125 | 917.2 | 125 | DR0–DR5 | Uplink de dados |
| 11 | BW125 | 917.4 | 125 | DR0–DR5 | Uplink de dados |
| 12 | BW125 | 917.6 | 125 | DR0–DR5 | Uplink de dados |
| 13 | BW125 | 917.8 | 125 | DR0–DR5 | Uplink de dados |
| 14 | BW125 | 918.0 | 125 | DR0–DR5 | Uplink de dados |
| 15 | BW125 | 918.2 | 125 | DR0–DR5 | Uplink de dados |
| 65 | BW500 | 917.5 | 500 | DR6 (SF8/BW500) | JOIN request (OTAA) / ADR |

#### Data Rates AU915

| DR | Modulação | SF | BW (kHz) | Payload máx. (bytes) | Uso típico |
|---|---|---|---|---|---|
| DR0 | LoRa | SF12 | 125 | 51 | Máximo alcance, mínima velocidade |
| DR1 | LoRa | SF11 | 125 | 51 | — |
| DR2 | LoRa | SF10 | 125 | 51 | — |
| DR3 | LoRa | SF9 | 125 | 115 | — |
| DR4 | LoRa | SF8 | 125 | 222 | — |
| **DR5** | **LoRa** | **SF7** | **125** | **222** | **Padrão neste projeto** |
| DR6 | LoRa | SF8 | 500 | 222 | Canal BW500 — JOIN/ADR |
| DR8–DR13 | LoRa | SF12–SF7 | 500 | — | Downlink RX2 |

> O firmware deste projeto usa `LMIC_setDrTxpow(DR_SF7, 14)` — DR5, SF7/BW125, potência 14 dBm — que oferece o maior throughput e menor tempo no ar dentro da sub-banda 1.

#### Janelas de downlink RX1 e RX2

Após cada uplink o dispositivo Class A abre duas janelas de recepção:

| Janela | Delay | Frequência | DR | Observação |
|---|---|---|---|---|
| RX1 | 1 s após uplink | Derivada do canal de uplink usado | Mesmo DR do uplink | Mapeamento definido pelo plano regional |
| **RX2** | **2 s após uplink** | **923.3 MHz** | **DR8 (SF12/BW500)** | **Fixo para AU915 — configurar no firmware e no ChirpStack** |

A janela RX2 é fixa em **923.3 MHz / SF12 / BW500 (DR8)** para AU915. No firmware LMiC isso é configurado manualmente em ABP (em OTAA o JOIN Accept configura automaticamente):

```cpp
LMIC.dn2Dr = DR_SF12CR;  // DR8 = SF12/BW500 — RX2 AU915
```

No Device Profile do ChirpStack (aba JOIN/ABP):
...
| **Device-status request frequency** | `916800000` |
...
Acesse **Device Profiles → Create**. São **8 perfis** no total: 4 para **ABP** e 4 para **OTAA**, cobrindo as duas sub-bandas e as duas revisões de parâmetros regionais. Configurações comuns a todos:
...
| **Class-B** | ☐ Não |
| **Class-C** | ✅ Sim — timeout `5s` |
| **Payload codec** | `Custom JavaScript codec functions` |

**Perfis ABP** (aba JOIN: `Device supports OTAA` ☐ desmarcado):

| Nome | Sub-banda | Revisão | RX1 delay | RX1 DR offset | RX2 DR | RX2 freq (Hz) | Factory freq (Hz) |
|---|---|---|---|---|---|---|---|
| `Devide Profile - AU915_0 - 1.0.3 - A (ABP)` | AU915_0 | A | `0` | `0` | `0` | `0` | `916800000` |
| `Devide Profile - AU915_1 - 1.0.3 - A (ABP)` | AU915_1 | A | `0` | `0` | `0` | `0` | `916800000` |
| `Devide Profile - AU915_0 - 1.0.3 - B (ABP)` | AU915_0 | B | `0` | `0` | `0` | `0` | `916800000` |
| `Devide Profile - AU915_1 - 1.0.3 - B (ABP)` | AU915_1 | B | `0` | `0` | `0` | `0` | `916800000` |

**Perfis OTAA** (aba JOIN: `Device supports OTAA` ✅ marcado):

| Nome | Sub-banda | Revisão |
|---|---|---|
| `Devide Profile - AU915_0 - 1.0.3 - A (OTAA)` | AU915_0 | A |
| `Devide Profile - AU915_1 - 1.0.3 - A (OTAA)` | AU915_1 | A |
| `Devide Profile - AU915_0 - 1.0.3 - B (OTAA)` | AU915_0 | B |
| `Devide Profile - AU915_1 - 1.0.3 - B (OTAA)` | AU915_1 | B |

> ⚠️ **Atenção — Class-C e downlinks em dispositivos Class A:** Todos os perfis acima têm **Class-C habilitado** (`Device supports Class-C → yes`). Dispositivos que operam como **Class A** (firmware LMiC padrão com OTAA ou ABP) **não recebem downlinks** quando esse campo está ativo, pois o ChirpStack passa a enviar os downlinks com timing `"immediately"` em vez de usar as janelas RX1/RX2 cronometradas. Se o seu dispositivo é Class A, desabilite o campo `Device supports Class-C` no Device Profile e limpe a fila do device após salvar. Consulte a seção [❌ Downlinks não chegam ao dispositivo Class A](#-downlinks-não-chegam-ao-dispositivo-class-a) para o diagnóstico completo.

---

#### Gateways

Acesse **Gateways → Create** e cadastre cada gateway. Habilite **Gateway Discovery** em todos:

| # | Nome | Descrição | Gateway EUI | IP | Modelo | Network Server | Service Profile | Gateway Profile |
|---|---|---|---|---|---|---|---|---|
| 001 | `001-gtw-rak831-001` | RAK831 RPi3-GPS (Multi Channel) | `b827ebfffea04db6` | `192.168.3.201` | Multi-canal | `ChirpStack-v3_NS` | `ChirpStack-v3_SP` | `ChirpStack-v3_GP_AU915_1` |
| 002 | `002-gtw-radioenge-001` | Radioenge RPi3-RD43HATGPS (Multi Channel) | `b827ebfffe158993` | `192.168.3.202` | Multi-canal | `ChirpStack-v3_NS` | `ChirpStack-v3_SP` | `ChirpStack-v3_GP_AU915_1` |
| 003 | `003-gtw-radioenge-002` | Radioenge RPi3-RD43HATGPS (Multi Channel) | `b827ebfffef81e69` | `192.168.3.203` | Multi-canal | `ChirpStack-v3_NS` | `ChirpStack-v3_SP` | `ChirpStack-v3_GP_AU915_1` |
| 004 | `004-gtw-radioenge-003` | Radioenge RPi3-RD43HATGPS (Multi Channel) | `b827ebfffe78ffce` | `192.168.3.204` | Multi-canal | `ChirpStack-v3_NS` | `ChirpStack-v3_SP` | `ChirpStack-v3_GP_AU915_1` |
| 005 | `005-gtw-elecrow-001` | Elecrow RPi4-GPS (Multi Channel) | `e45f01fffe10e12e` | `192.168.3.205` | Multi-canal | `ChirpStack-v3_NS` | `ChirpStack-v3_SP` | `ChirpStack-v3_GP_AU915_1` |
| 006 | `006-gtw-dragino_lg02-001` | Dragino LG02 (Dual Channel) | `a840411b7dcc4150` | `10.130.1.1` | Dual-canal | `ChirpStack-v3_NS` | `ChirpStack-v3_SP` | `ChirpStack-v3_GP_AU915_1` |
...
Acesse **Device profile templates → Add** e cadastre os **4 templates** — um por combinação de revisão de parâmetros regionais (A/B) e modo de ativação (ABP/OTAA):

| ID | Nome | Revisão | Ativação | Vendor | Firmware | Class-C | Codec |
|---|---|---|---|---|---|---|---|
| `Devide_Profile_Template_AU915_1_0_3_A_ABP` | `Devide Profile Template - AU915 - 1.0.3 - A (ABP)` | A | ABP | `AdailSilva-IoT` | `1.0` | ✅ Sim — `5s` | JavaScript |
| `Devide_Profile_Template_AU915_1_0_3_A_OTAA` | `Devide Profile Template - AU915 - 1.0.3 - A (OTAA)` | A | OTAA | `AdailSilva-IoT` | `1.0` | ✅ Sim — `5s` | JavaScript |
| `Devide_Profile_Template_AU915_1_0_3_B_ABP` | `Devide Profile Template - AU915 - 1.0.3 - B (ABP)` | B | ABP | `AdailSilva-IoT` | `1.0` | ✅ Sim — `5s` | JavaScript |
| `Devide_Profile_Template_AU915_1_0_3_B_OTAA` | `Devide Profile Template - AU915 - 1.0.3 - B (OTAA)` | B | OTAA | `AdailSilva-IoT` | `1.0` | ✅ Sim — `5s` | JavaScript |

Descrições dos templates:

| Template | Descrição |
|---|---|
| `A (ABP)` | `Devide Profile Template Region AU915 - MAC version LoRaWAN 1.0.3 - Regional parameters revision A (ABP)` |
| `A (OTAA)` | `Devide Profile Template Region AU915 - MAC version LoRaWAN 1.0.3 - Regional parameters revision A (OTAA)` |
| `B (ABP)` | `Devide Profile Template Region AU915 - MAC version LoRaWAN 1.0.3 - Regional parameters revision B (ABP)` |
| `B (OTAA)` | `Devide Profile Template Region AU915 - MAC version LoRaWAN 1.0.3 - Regional parameters revision B (OTAA)` |

---

#### Device Profiles

Acesse **Device profiles → Add**. São **8 perfis** no total: 4 para **ABP** e 4 para **OTAA**, cobrindo as duas sub-bandas e as duas revisões de parâmetros regionais.

**Perfis ABP** (`Supports OTAA → no`):

| Nome | Sub-banda | Revisão | OTAA | Class-B | Class-C | Timeout C | Codec |
|---|---|---|---|---|---|---|---|
| `Devide Profile - AU915_0 - 1.0.3 - A (ABP)` | AU915_0 | A | ☐ Não | ☐ Não | ✅ Sim | 5s | JavaScript |
| `Devide Profile - AU915_1 - 1.0.3 - A (ABP)` | AU915_1 | A | ☐ Não | ☐ Não | ✅ Sim | 5s | JavaScript |
| `Devide Profile - AU915_0 - 1.0.3 - B (ABP)` | AU915_0 | B | ☐ Não | ☐ Não | ✅ Sim | 5s | JavaScript |
| `Devide Profile - AU915_1 - 1.0.3 - B (ABP)` | AU915_1 | B | ☐ Não | ☐ Não | ✅ Sim | 5s | JavaScript |

**Perfis OTAA** (`Supports OTAA → yes`):

| Nome | Sub-banda | Revisão | OTAA | Class-B | Class-C | Timeout C | Codec |
|---|---|---|---|---|---|---|---|
| `Devide Profile - AU915_0 - 1.0.3 - A (OTAA)` | AU915_0 | A | ✅ Sim | ☐ Não | ✅ Sim | 5s | JavaScript |
| `Devide Profile - AU915_1 - 1.0.3 - A (OTAA)` | AU915_1 | A | ✅ Sim | ☐ Não | ✅ Sim | 5s | JavaScript |
| `Devide Profile - AU915_0 - 1.0.3 - B (OTAA)` | AU915_0 | B | ✅ Sim | ☐ Não | ✅ Sim | 5s | JavaScript |
| `Devide Profile - AU915_1 - 1.0.3 - B (OTAA)` | AU915_1 | B | ✅ Sim | ☐ Não | ✅ Sim | 5s | JavaScript |

Descrições dos perfis:

| Perfil | Descrição |
|---|---|
| `AU915_0 - 1.0.3 - A (ABP)` | `Devide Profile Region AU915 - Region configuration 0 - MAC version LoRaWAN 1.0.3 - Regional parameters revision A (ABP)` |
| `AU915_1 - 1.0.3 - A (ABP)` | `Devide Profile Region AU915 - Region configuration 1 - MAC version LoRaWAN 1.0.3 - Regional parameters revision A (ABP)` |
| `AU915_0 - 1.0.3 - B (ABP)` | `Devide Profile Region AU915 - Region configuration 0 - MAC version LoRaWAN 1.0.3 - Regional parameters revision B (ABP)` |
| `AU915_1 - 1.0.3 - B (ABP)` | `Devide Profile Region AU915 - Region configuration 1 - MAC version LoRaWAN 1.0.3 - Regional parameters revision B (ABP)` |
| `AU915_0 - 1.0.3 - A (OTAA)` | `Devide Profile Region AU915 - Region configuration 0 - MAC version LoRaWAN 1.0.3 - Regional parameters revision A (OTAA)` |
| `AU915_1 - 1.0.3 - A (OTAA)` | `Devide Profile Region AU915 - Region configuration 1 - MAC version LoRaWAN 1.0.3 - Regional parameters revision A (OTAA)` |
| `AU915_0 - 1.0.3 - B (OTAA)` | `Devide Profile Region AU915 - Region configuration 0 - MAC version LoRaWAN 1.0.3 - Regional parameters revision B (OTAA)` |
| `AU915_1 - 1.0.3 - B (OTAA)` | `Devide Profile Region AU915 - Region configuration 1 - MAC version LoRaWAN 1.0.3 - Regional parameters revision B (OTAA)` |

> ⚠️ **Atenção — Class-C e downlinks em dispositivos Class A:** Todos os perfis acima têm **Class-C habilitado** (`Supports Class-C → yes`). Dispositivos que operam como **Class A** (firmware LMiC padrão com OTAA ou ABP) **não recebem downlinks** quando esse campo está ativo, pois o ChirpStack passa a enviar os downlinks com timing `"immediately"` em vez de usar as janelas RX1/RX2 cronometradas. Se o seu dispositivo é Class A, desabilite o campo `Supports Class-C` no Device Profile e limpe a fila do device após salvar. Consulte a seção [❌ Downlinks não chegam ao dispositivo Class A](#-downlinks-não-chegam-ao-dispositivo-class-a) para o diagnóstico completo.

---

#### Gateways

Acesse **Gateways → Add** e cadastre cada gateway. No v4 não há vinculação a Network Server, Service Profile ou Gateway Profile — basta informar o EUI e o nome:

| # | Nome | Descrição | Gateway EUI | IP | Modelo |
|---|---|---|---|---|---|
| 001 | `001-gtw-rak831-001` | RAK831 RPi3-GPS (Multi Channel) | `b827ebfffea04db6` | `192.168.3.201` | Multi-canal |
| 002 | `002-gtw-radioenge-001` | Radioenge RPi3-RD43HATGPS (Multi Channel) | `b827ebfffe158993` | `192.168.3.202` | Multi-canal |
| 003 | `003-gtw-radioenge-002` | Radioenge RPi3-RD43HATGPS (Multi Channel) | `b827ebfffef81e69` | `192.168.3.203` | Multi-canal |
| 004 | `004-gtw-radioenge-003` | Radioenge RPi3-RD43HATGPS (Multi Channel) | `b827ebfffe78ffce` | `192.168.3.204` | Multi-canal |
| 005 | `005-gtw-elecrow-001` | Elecrow RPi4-GPS (Multi Channel) | `e45f01fffe10e12e` | `192.168.18.205` | Multi-canal |
| 006 | `006-gtw-dragino_lg02-001` | Dragino LG02 (Dual Channel) | `a840411b7dcc4150` | `10.130.1.1` | Dual-canal |

> No v4 o gateway se conecta ao cluster via UDP e o ChirpStack o reconhece automaticamente pelo EUI quando começa a enviar pacotes. O vínculo com a região (AU915_1) é feito pelo `chirpstack.toml` no servidor, não pelo cadastro do gateway.
...
| **Device Profile Templates** | Não existe | ✅ 4 templates (ABP-A, ABP-B, OTAA-A, OTAA-B) |
| **Device Profiles** | 8 perfis (4 ABP + 4 OTAA) | 8 perfis (4 ABP + 4 OTAA) |
...
| **Separação ABP / OTAA nos perfis** | Campo `Device supports OTAA` na aba JOIN | Campo `Supports OTAA` no perfil |

---

## Classes de dispositivos LoRaWAN — A, B e C

O padrão LoRaWAN define três classes de operação para dispositivos — **A**, **B** e **C** — que diferem fundamentalmente na forma como e quando o dispositivo pode receber mensagens do servidor (downlinks). A escolha da classe impacta diretamente o consumo de energia, a latência dos downlinks e a complexidade do firmware.

Todo dispositivo LoRaWAN implementa obrigatoriamente a **Class A** como base. As classes B e C são extensões opcionais que acrescentam capacidades de recepção adicionais.

---

### Class A — Aloha

**Nome:** *Aloha* — referência ao protocolo de acesso aleatório ao meio ALOHA, desenvolvido na Universidade do Havaí nos anos 1970, que opera sem coordenação central: cada estação transmite quando tem dados, sem verificar se o canal está livre.

**Como funciona:**

O dispositivo Class A só transmite e recebe por iniciativa própria. Após cada uplink, ele abre duas janelas de recepção curtas e sequenciais — **RX1** e **RX2** — e só durante essas janelas o servidor pode enviar um downlink. Fora delas, o rádio permanece desligado.

```
Dispositivo:  [uplink] ──── 1s ──── [RX1] ──── 1s ──── [RX2] ────── (rádio desligado) ──────
Servidor:                            ↑                    ↑
                                     Pode enviar          Pode enviar
                                     downlink aqui        downlink aqui (frequência e DR fixos)
```

- **RX1** abre 1 segundo após o fim do uplink, na mesma frequência e DR do uplink (mapeados pelo plano regional).
- **RX2** abre 2 segundos após o fim do uplink, em frequência e DR fixos definidos pelo plano regional (AU915: 923.3 MHz / SF12/BW500).
- Se o servidor enviar downlink em RX1, RX2 não é aberta.
- Entre transmissões o dispositivo pode entrar em deep sleep — consumo na ordem de **µA**.

**Casos de uso ideais:** sensores de baixo consumo com bateria (temperatura, umidade, nível de água, medidores de energia), qualquer aplicação onde downlinks esporádicos são aceitáveis.

**Firmware neste projeto:** o MCCI LMiC implementa Class A nativamente. Todo o código deste repositório opera em Class A.

---

### Class B — Beacon

**Nome:** *Beacon* — referência ao sinal de farol (*beacon*) que o gateway transmite periodicamente para sincronizar o relógio dos dispositivos, permitindo que eles abram janelas de recepção em momentos previsíveis.

**Como funciona:**

Class B estende Class A adicionando **ping slots** — janelas de recepção abertas em intervalos regulares e sincronizados com um beacon transmitido pelo gateway a cada 128 segundos. O servidor conhece o schedule exato de cada dispositivo e pode enfileirar downlinks para serem entregues no próximo ping slot disponível.

```
Gateway:     ──[beacon]────────────────────────[beacon]────────────────────────[beacon]──
                  │                                │
Dispositivo: ──[uplink][RX1][RX2]──[ping]──[ping]──[uplink][RX1][RX2]──[ping]──[ping]──
                                    ↑       ↑                            ↑       ↑
                              Janelas abertas                       Janelas abertas
                              mesmo sem uplink                      mesmo sem uplink
```

- O gateway sincroniza todos os dispositivos Class B com o beacon GPS.
- O dispositivo acorda nos ping slots mesmo sem ter transmitido — o rádio RX fica ligado periodicamente.
- Latência de downlink previsível e limitada pelo período entre ping slots (configurável: 1s a 128s).
- Consumo maior que Class A por causa dos acordes periódicos, mas menor que Class C.

**Casos de uso ideais:** atuadores que precisam receber comandos com latência previsível mas ainda valorizam economia de energia — válvulas remotas, rastreadores com atualização de rota periódica, displays de informação.

**Firmware neste projeto:** o MCCI LMiC possui suporte experimental a Class B, mas **não está habilitado** nos firmwares deste repositório. Para Class B em produção, recomenda-se a **LoRaMAC-Node**.

---

### Class C — Continuous

**Nome:** *Continuous* — o dispositivo mantém o rádio em modo de recepção **continuamente**, exceto durante as transmissões de uplink.

**Como funciona:**

Class C estende Class A: após cada uplink e janela RX1, o dispositivo não fecha o rádio — ele permanece em RX2 continuamente até o próximo uplink. O servidor pode enviar um downlink a qualquer momento com latência mínima (tipicamente < 1 segundo).

```
Dispositivo: ──[uplink][RX1][════════════════ RX2 contínuo ════════════════][uplink][RX1]──
Servidor:                    ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
                              Pode enviar downlink a qualquer momento
```

- O rádio receptor fica ligado o tempo todo — consumo muito maior que Class A ou B.
- Class C é indicada apenas para dispositivos com alimentação contínua (tomada, painel solar com bateria grande).
- Latência de downlink mínima entre as três classes.
- Não há sincronização por beacon — o servidor simplesmente envia quando quiser.

**Casos de uso ideais:** atuadores com alimentação contínua que exigem resposta imediata — relés, dimmers, sirenes de alarme, portões automáticos.

**Firmware neste projeto:** os perfis de device no ChirpStack v3 têm Class-C cadastrado como `yes` por padrão (campo herdado do template). No ChirpStack v4 os perfis foram configurados com `Class-C = no`. Em ambos os casos, o **firmware LMiC opera em Class A** independentemente do que está no perfil do servidor — o perfil apenas instrui o ChirpStack sobre como tratar os downlinks para aquele device.

> ⚠️ **Atenção — Class-C no perfil vs Class-C no firmware:** se o Device Profile do ChirpStack tiver `Class-C = yes` mas o firmware do dispositivo for Class A (LMiC padrão), o ChirpStack enviará downlinks com timing `"immediately"` (sem aguardar RX1/RX2), e o dispositivo **nunca os receberá**. Mantenha sempre o perfil do servidor alinhado com a classe implementada no firmware. Consulte a seção [❌ Downlinks não chegam ao dispositivo Class A](#-downlinks-não-chegam-ao-dispositivo-class-a) para o diagnóstico completo.

---

### Comparativo das três classes

| Característica | Class A | Class B | Class C |
|---|---|---|---|
| **Nome completo** | Aloha | Beacon | Continuous |
| **Recepção** | Apenas após uplink (RX1 + RX2) | RX1 + RX2 + ping slots periódicos | RX2 contínuo + RX1 após uplink |
| **Latência de downlink** | Alta (até o próximo uplink) | Previsível (período do ping slot) | Mínima (< 1 s) |
| **Consumo de energia** | ⚡ Muito baixo | ⚡⚡ Moderado | ⚡⚡⚡ Alto |
| **Alimentação típica** | Bateria | Bateria / solar | Contínua |
| **Sincronização** | Não necessária | GPS beacon do gateway | Não necessária |
| **Complexidade do firmware** | Baixa | Alta | Média |
| **Suporte obrigatório** | ✅ Todos os devices | ☐ Opcional | ☐ Opcional |
| **Base** | — | Estende Class A | Estende Class A |

---

### Bibliotecas e suporte por classe

A escolha da biblioteca de firmware determina quais classes estão disponíveis:

#### MCCI Arduino LMiC

A biblioteca utilizada neste projeto — **MCCI LoRaWAN LMIC** — implementa Class A e possui suporte experimental a Class B. **Class C não é suportada.**

| Classe | Suporte | Observação |
|---|---|---|
| **Class A** | ✅ Completo | Usado em todos os firmwares deste projeto |
| **Class B** | ⚠️ Experimental | Disponível, mas não testado em produção |
| **Class C** | ❌ Não suportado | — |

```cpp
// Class A é o padrão — nenhuma configuração adicional necessária
// O LMiC opera em Class A automaticamente após LMIC_setSession()
```

Repositório: [github.com/mcci-catena/arduino-lmic](https://github.com/mcci-catena/arduino-lmic)

Versões utilizadas neste projeto:
- **v3.3.0** (recomendada): [github.com/AdailSilva/IBM-LMIC-LoRaWAN-MAC-in-C-library-v3.3.0](https://github.com/AdailSilva/IBM-LMIC-LoRaWAN-MAC-in-C-library-v3.3.0)
- **v6.0.1** (problemática com ABP LoRaWAN 1.0.x): [github.com/AdailSilva/IBM-LMIC-LoRaWAN-MAC-in-C-library-v6.0.1](https://github.com/AdailSilva/IBM-LMIC-LoRaWAN-MAC-in-C-library-v6.0.1)

#### LoRaMAC-Node

Para quem precisa das **três classes completas** — especialmente Class B com sincronização por beacon e Class C com recepção contínua — a alternativa recomendada é a **LoRaMAC-Node**, a implementação de referência oficial do stack LoRaWAN mantida pela Semtech e pela LoRa Alliance.

| Classe | Suporte | Observação |
|---|---|---|
| **Class A** | ✅ Completo | Base obrigatória |
| **Class B** | ✅ Completo | Beacon sincronizado com GPS gateway |
| **Class C** | ✅ Completo | Recepção contínua em RX2 |

A LoRaMAC-Node é escrita em C puro e projetada para portabilidade — funciona em STM32, ESP32, NRF52, RP2040 e outras plataformas com adaptação da camada HAL (Hardware Abstraction Layer).

```c
// Exemplo de configuração de classe na LoRaMAC-Node
MlmeReq_t mlmeReq;
mlmeReq.Type = MLME_DEVICE_TIME;  // sincroniza relógio para Class B

// Após sincronização, solicitar ping slots (Class B)
MibRequestConfirm_t mibReq;
mibReq.Type = MIB_DEVICE_CLASS;
mibReq.Param.Class = CLASS_B;     // CLASS_A, CLASS_B ou CLASS_C
LoRaMacMibSetRequestConfirm(&mibReq);
```

Repositório: [github.com/Lora-net/LoRaMac-node](https://github.com/Lora-net/LoRaMac-node)

#### Quando usar cada biblioteca

| Critério | MCCI LMiC | LoRaMAC-Node |
|---|---|---|
| **Classe necessária** | A (ou B experimental) | A, B ou C |
| **Plataforma** | Arduino (AVR, ESP32, SAMD) | STM32, ESP32, NRF52, RP2040 |
| **Integração Arduino IDE** | ✅ Nativa | ☐ Requer adaptação |
| **Complexidade** | Baixa | Alta |
| **Maturidade** | Alta (amplamente usada) | Alta (referência oficial) |
| **Downlinks imediatos (Class C)** | ❌ | ✅ |
| **Ping slots (Class B)** | ⚠️ Experimental | ✅ |

> Para o contexto deste projeto — ESP32 + RFM95W com sensores de medição de energia operando em Class A — o **MCCI LMiC** é a escolha correta. A LoRaMAC-Node seria indicada se o projeto evoluísse para atuadores com requisito de resposta imediata (Class C) ou para dispositivos de rastreamento com downlinks periódicos previsíveis (Class B).

---

## Envio de Downlinks via MQTT

O ChirpStack expõe uma integração MQTT que permite tanto **monitorar eventos** (uplinks, joins, status) quanto **disparar downlinks** para dispositivos — tudo via publicação e assinatura de tópicos MQTT. Qualquer cliente MQTT pode ser usado: linha de comando (`mosquitto_pub` / `mosquitto_sub`), ferramentas gráficas como [MQTTX](https://mqttx.app/) ou [MQTT Explorer](https://mqtt-explorer.com/), ou código em qualquer linguagem.

Esta seção documenta os comandos completos para Linux, cobrindo os dois cenários deste projeto: **ChirpStack v3** (broker sem TLS, porta `1883`) e **ChirpStack v4** (broker com TLS mútuo, porta `8883`).

---

### Instalação do cliente MQTT

O pacote `mosquitto-clients` fornece os dois utilitários de linha de comando necessários: `mosquitto_pub` (publicar) e `mosquitto_sub` (assinar).
...
Para ferramentas gráficas, as opções mais populares são:

```bash
# MQTTX — cliente gráfico multiplataforma (AppImage no Linux)
wget https://github.com/emqx/MQTTX/releases/latest/download/MQTTX-linux-x64.AppImage
chmod +x MQTTX-linux-x64.AppImage
./MQTTX-linux-x64.AppImage

# MQTT Explorer — alternativa leve (Snap)
sudo snap install mqtt-explorer
```

---

### Conceitos básicos — pub/sub e tópicos

O MQTT opera no modelo **publish/subscribe**: clientes publicam mensagens em tópicos e outros clientes assinados recebem essas mensagens. No ChirpStack os tópicos seguem o padrão:

```
# Eventos do dispositivo (uplink, join, status, ack, error...)
application/{APPLICATION_ID}/device/{DEV_EUI}/event/{event}

# Comandos para o dispositivo (downlink)
application/{APPLICATION_ID}/device/{DEV_EUI}/command/down
```

O payload de downlink deve ser um JSON com o campo `data` em **Base64**. O ChirpStack descriptografa e entrega ao device na próxima janela RX disponível (RX1 ou RX2 para Class A).

```bash
# Converter texto para Base64 (Linux)
echo -n "payload aqui" | base64

# Converter bytes hex para Base64
printf '\x01\x02\x03' | base64

# Converter Base64 de volta para texto (verificação)
echo "cGF5bG9hZCBhcXVp" | base64 -d
```

---

### Disparar downlink — ChirpStack v3 (sem TLS)

O v3 usa Mosquitto sem TLS — conexão direta na porta `1883` via port-forward do Kubernetes ou pelo endereço interno do cluster.

**Valores deste projeto:**

| Campo | Valor |
|---|---|
| Application ID | `1` (numérico no v3) |
| DevEUI | `08fdbe4bc604e8f8` |
| Broker | `chirpstack-v3-mosquitto-service.chirpstack-v3.svc.cluster.local:1883` |

**Passo 1 — Port-forward do broker (se acessando de fora do cluster):**

```bash
kubectl port-forward \
  svc/chirpstack-v3-mosquitto-service \
  1883:1883 \
  -n chirpstack-v3
```

**Passo 2 — Disparar o downlink:**

```bash
# Payload: texto "AdailSilva-IoT" em Base64
PAYLOAD=$(echo -n "AdailSilva-IoT" | base64)

mosquitto_pub \
  -h localhost \
  -p 1883 \
  -t "application/1/device/08fdbe4bc604e8f8/command/down" \
  -m "{
    \"devEui\": \"08fdbe4bc604e8f8\",
    \"confirmed\": false,
    \"fPort\": 1,
...
**Payload com bytes arbitrários (ex: comando `0x01 0x02 0x03`):**

```bash
PAYLOAD=$(printf '\x01\x02\x03' | base64)
...
**Downlink confirmado (device deve enviar ACK):**
...
mosquitto_pub \
  ...
  -m "{
    ...
    \"confirmed\": true,
    \"fPort\": 1,
    \"data\": \"$(echo -n 'ping' | base64)\"
  }"
```

---

### Disparar downlink — ChirpStack v4 (com TLS)

O v4 usa Mosquitto com TLS na porta `8883` e autenticação por usuário/senha. O certificado CA interno do cluster é necessário para validar a conexão.

**Valores deste projeto:**

| Campo | Valor |
|---|---|
| Application ID | `56560a65-2fb8-444c-a1ee-bd0ee4be0946` (UUID no v4) |
| DevEUI | `08fdbe4bc604e8f8` |
| Broker externo | `chirpstack-v4.adailsilva.com.br:8883` |
| Usuário MQTT | `adailsilva` |
| CA cert | `~/chirpstack/mqtt-certs/ca.crt` |

**Passo 1 — Obter o certificado CA (se ainda não tiver localmente):**

```bash
# Copiar o CA do cluster para a máquina local
mkdir -p ~/chirpstack/mqtt-certs

kubectl get secret chirpstack-mqtt-ca \
  -n chirpstack-v4 \
  -o jsonpath='{.data.ca\.crt}' | base64 -d \
  > ~/chirpstack/mqtt-certs/ca.crt

# Verificar o certificado
openssl x509 -in ~/chirpstack/mqtt-certs/ca.crt -noout -subject -dates
```

**Passo 2 — Testar conectividade TLS:**

```bash
# Verificar handshake TLS com o broker
openssl s_client \
  -connect chirpstack-v4.adailsilva.com.br:8883 \
  -CAfile ~/chirpstack/mqtt-certs/ca.crt \
  -quiet 2>/dev/null | head -5
```

**Passo 3 — Disparar o downlink via TLS:**
...
> A flag `-d` ativa o modo debug — exibe o handshake MQTT e confirma que a mensagem foi entregue ao broker.

**Via port-forward (alternativa sem expor o broker externamente):**

```bash
# Em um terminal: abrir o tunnel
kubectl port-forward \
  svc/chirpstack-v4-mosquitto-service \
  8883:8883 \
  -n chirpstack-v4

# Em outro terminal: disparar o downlink
mosquitto_pub \
  -h localhost \
  -p 8883 \
  --cafile ~/chirpstack/mqtt-certs/ca.crt \
  ...
  -m "{
    ...
    \"confirmed\": false,
    \"fPort\": 1,
    \"data\": \"$(echo -n 'AdailSilva-IoT' | base64)\"
  }"
```

> `--insecure` é necessário via port-forward porque `localhost` não está nos SANs do certificado. Mantém a criptografia TLS ativa — desabilita apenas a validação do hostname. **Nunca use `--insecure` ao conectar pelo endereço real do cluster.**

**Payload com bytes arbitrários:**
...
mosquitto_pub \
  ...
  -m "{
    ...
    \"confirmed\": false,
    \"fPort\": 1,
    \"data\": \"$(printf '\x01\x02\x03' | base64)\"
  }"
```

**Verificar na fila do ChirpStack:**

Após publicar, o downlink ficará enfileirado até o próximo uplink do dispositivo (Class A). Verifique em:

> ChirpStack → Applications → `devices_au915` → Devices → `ESP32 RF95W (ABP)` → **Queue**

O item deve aparecer na fila com status `pending`. Ao transmitir o próximo uplink, o device receberá o downlink na janela RX1 ou RX2 e o item sairá da fila.

---

### Monitorar uplinks e eventos via MQTT

Além de disparar downlinks, o `mosquitto_sub` permite monitorar em tempo real todos os eventos do ChirpStack.

**ChirpStack v3 — sem TLS:**

```bash
# Port-forward primeiro
kubectl port-forward svc/chirpstack-v3-mosquitto-service 1883:1883 -n chirpstack-v3

# Todos os eventos de todos os devices da application
mosquitto_sub \
  -h localhost \
  -p 1883 \
  -t "application/1/#" \
...
mosquitto_sub \
  -h localhost \
  -p 1883 \
  -t "application/1/device/+/event/up" \
  -v
...
mosquitto_sub \
  -h localhost \
  -p 1883 \
  -t "application/1/device/+/event/join" \
  -v
```
...
# Apenas uplinks do device específico
mosquitto_sub \
  -h chirpstack-v4.adailsilva.com.br \
  -p 8883 \
  --cafile ~/chirpstack/mqtt-certs/ca.crt \
  -u "adailsilva" \
  -P "H@cker101" \
  -t "application/${APP_ID}/device/08fdbe4bc604e8f8/event/up" \
  -v

# Uplinks de todos os devices + formatação JSON legível
mosquitto_sub \
  -h chirpstack-v4.adailsilva.com.br \
  -p 8883 \
  --cafile ~/chirpstack/mqtt-certs/ca.crt \
  -u "adailsilva" \
  -P "H@cker101" \
  -t "application/${APP_ID}/device/+/event/up" \
...
### Referência de tópicos MQTT

**ChirpStack v3:**

| Tópico | Direção | Descrição |
|---|---|---|
| `application/{id}/device/{eui}/event/up` | ← Servidor | Uplink recebido |
| `application/{id}/device/{eui}/event/join` | ← Servidor | JOIN OTAA aceito |
| `application/{id}/device/{eui}/event/ack` | ← Servidor | ACK de downlink confirmado |
| `application/{id}/device/{eui}/event/status` | ← Servidor | Status do device (bateria, link margin) |
| `application/{id}/device/{eui}/event/error` | ← Servidor | Erro (payload inválido, etc.) |
| `application/{id}/device/{eui}/command/down` | → Servidor | **Enfileirar downlink** |

**ChirpStack v4** (mesma estrutura, Application ID em UUID):

| Tópico | Direção | Descrição |
|---|---|---|
| `application/{uuid}/device/{eui}/event/up` | ← Servidor | Uplink recebido |
| `application/{uuid}/device/{eui}/event/join` | ← Servidor | JOIN OTAA aceito |
| `application/{uuid}/device/{eui}/event/ack` | ← Servidor | ACK de downlink confirmado |
| `application/{uuid}/device/{eui}/event/txack` | ← Servidor | Confirmação de transmissão pelo gateway |
| `application/{uuid}/device/{eui}/event/log` | ← Servidor | Log de erro ou aviso |
| `application/{uuid}/device/{eui}/command/down` | → Servidor | **Enfileirar downlink** |

**Campos do JSON de downlink:**

| Campo | Tipo | Obrigatório | Descrição |
|---|---|---|---|
| `devEui` | string | ✅ | Deve bater com o DevEUI do tópico |
| `confirmed` | bool | ✅ | `true` = downlink confirmado (device envia ACK) |
| `fPort` | int | ✅ | FPort > 0 — use `1` para dados genéricos |
| `data` | string | ✅ | Payload em Base64 (plaintext — ChirpStack cifra) |
| `object` | object | ☐ | Alternativa ao `data` quando há codec JavaScript |

> ⚠️ **Class A e timing do downlink:** dispositivos Class A só recebem downlinks nas janelas RX1/RX2, que abrem após cada uplink. O downlink ficará na fila do ChirpStack até o próximo uplink do dispositivo — em intervalos de `TX_INTERVAL_S` segundos (10 s neste projeto). Não há garantia de entrega imediata.
...
| `GH_PAT` | Personal Access Token do GitHub com escopos `repo` e `workflow` | Gerado em **GitHub → Settings → Developer settings → Personal access tokens → Tokens (classic)** — obrigatório também para o checkout em repositórios privados e para push de tags em repositórios com workflows |
...
5. Preencha:
   ...
   - **Scopes:** marque ✅ `repo` e ✅ `workflow` — `repo` dá permissão de leitura/escrita no repositório e em tags; `workflow` é obrigatório porque o repositório contém `.github/workflows/` e o GitHub rejeita qualquer push de tag que referencie commits com arquivos de workflow sem esse escopo, com o erro `refusing to allow a Personal Access Token to create or update workflow`
...
> O pipeline usa o `GH_PAT` em dois momentos: no step de `Checkout` (`token: ${{ secrets.GH_PAT }}`), onde é obrigatório para clonar repositórios privados — sem ele o runner recebe o erro `Input required and not supplied: token` e o job falha imediatamente — e no step **Criar tag Git**, que executa `git push origin <versão>`. O `GITHUB_TOKEN` padrão do Actions não tem permissão de escrita em tags nem de leitura em repositórios privados de outros contextos, por isso o `GH_PAT` é indispensável em ambos os casos.
>
> ⚠️ **Repositório privado:** o token precisa estar cadastrado como Secret no próprio repositório (ou na organização) em **Settings → Secrets and variables → Actions**. Criá-lo apenas em **Developer settings** não é suficiente — o runner não consegue acessá-lo automaticamente a partir daí.
...
### ❌ Downlinks não chegam ao dispositivo Class A

Dispositivos operando em **Class A** (firmware LMiC padrão, OTAA ou ABP) só recebem downlinks nas janelas **RX1** e **RX2**, que abrem em intervalos cronometrados de 1 s e 2 s após o fim de cada uplink. Se o ChirpStack estiver enviando downlinks com timing `"immediately"`, o gateway transmite instantaneamente — sem aguardar essas janelas — e o device nunca recebe.

**Sintoma no JSON de downlink do ChirpStack:**
...
Em vez do correto para Class A:
...
**Causa raiz:** O Device Profile está com `Device supports Class-C → yes`. Quando esse campo está ativo, o ChirpStack trata o device como Class C e usa timing imediato para todos os downlinks de dados — mesmo que o firmware seja Class A.

> 💡 **Por que o JOIN OTAA funciona mesmo com Class C ativo?** O JOIN Accept usa um mecanismo de timing próprio (`immediately` na janela JACC1/JACC2), independente da classe configurada no Device Profile. Por isso o JOIN sempre funciona, mas os downlinks de dados subsequentes falham.

---

**Solução — passo a passo:**

**1. Corrija o Device Profile:**

Acesse **Device Profiles → seu profile** e na aba **General** (v4) ou **Class-C** (v3):

| Campo | Valor atual (incorreto) | Valor correto |
|---|---|---|
| `Device supports Class-C` / `Supports Class-C` | ✅ yes | ☐ no |
| `Class-C confirmed downlink timeout` | `5` | *(campo some ao desabilitar)* |

Salve o profile.

**2. Limpe a fila de downlinks pendentes:**

Acesse **Devices → seu device → Queue** e delete todos os itens. Os pacotes enfileirados antes da correção do profile ainda carregam o timing `immediately` e precisam ser removidos.

**3. Reinicie o device:**

Execute um reset físico (power cycle) no ESP32 ou outro microcontrolador para forçar um novo JOIN OTAA limpo.

**4. Enfileire um novo downlink:**

Acesse **Devices → seu device → Queue**, adicione um novo item e aguarde o próximo uplink. O JSON de downlink agora deve conter:
...
E o Serial Monitor deve exibir:

```
EV_TXCOMPLETE (includes waiting for RX windows)
  Downlink received — 1 byte(s) on FPort 1: 01
```
...
|---|---|---|
| Timing do downlink | JSON em **Devices → Queue** após envio | `"delay": {"delay": "1s"}` |
| Device Profile Class | **Device Profiles → seu profile** | `Supports Class-C → no` |
| Downlink na fila | **Devices → seu device → Queue** | Item enfileirado presente |
| Frame do gateway | **Gateways → seu gateway → Frames** | Seta ↑ (uplink) + seta ↓ (downlink) |
| RX2 no Serial Monitor | Saída do firmware LMiC | `RXMODE_SINGLE, freq=923300000, SF=12, BW=500` |

> ⚠️ Esta situação pode ocorrer tanto no ChirpStack v3 quanto no v4. Em ambas as versões, o campo `Device supports Class-C` deve ser desabilitado para dispositivos que operam em Class A.

---

### ❌ Downlink descartado — payload excede o tamanho máximo (`DOWNLINK_PAYLOAD_SIZE`)

O ChirpStack descarta o item da fila e registra um erro quando o payload do downlink é maior do que o limite permitido pelo Data Rate configurado no dispositivo.

**Sintoma no log de eventos do device (`level: ERROR`):**
...
```json
{
    ...
    "context": {
        "item_size": "383",
        "queue_item_id": "fcd796ba-b2a0-4672-8c42-3904adf9f0f8",
        "max_payload_size": "242"
    }
}
```

Os campos relevantes são `item_size` (tamanho do payload enviado) e `max_payload_size` (limite para o DR atual do device).

**Causa raiz:** O payload enfileirado excede o limite máximo de bytes permitido pela especificação LoRaWAN para o Data Rate em uso. Este limite é fixo — não pode ser alterado no ChirpStack nem no firmware.

---

**Limites por Data Rate — AU915:**

| DR | SF | BW | Payload máximo |
|---|---|---|---|
| DR0 | SF12 | BW125 | **11 bytes** |
| DR1 | SF11 | BW125 | **53 bytes** |
| DR2 | SF10 | BW125 | **125 bytes** |
| DR3 | SF9  | BW125 | **242 bytes** |
| DR4 | SF8  | BW125 | **242 bytes** |
| DR5 | SF7  | BW125 | **242 bytes** ← padrão com `LMIC_setDrTxpow(DR_SF7, 14)` |
| DR6 | SF8  | BW500 | **242 bytes** |

> O firmware deste projeto usa `DR_SF7` (DR5), cujo limite é **242 bytes**. Qualquer downlink enfileirado com mais de 242 bytes será descartado automaticamente pelo ChirpStack.

---

**Soluções:**

**Opção A — Fragmentar o payload (recomendado para dados estruturados):**

Divida o conteúdo em múltiplos downlinks de no máximo 200 bytes cada (com margem de segurança de ~17%):

```
payload total: 383 bytes
─────────────────────────
chunk 1: bytes   0–199  → downlink FPort 1, enfileire primeiro
chunk 2: bytes 200–382  → downlink FPort 1, enfileire segundo
```

O device deve remontar os fragmentos na aplicação usando o FPort ou um byte de controle no início do payload.

**Opção B — Compactar o payload (recomendado para dados de sensores):**

Substitua JSON ou texto por formato binário compacto. Dados típicos de medidores de energia (tensão, corrente, potência, energia acumulada, fator de potência) cabem facilmente em menos de 20 bytes:

```
// Exemplo: 5 grandezas em 14 bytes
[0–1]  tensão      uint16  × 0.1 V   → 2 bytes
[2–3]  corrente    uint16  × 0.01 A  → 2 bytes
[4–5]  potência    uint16  W         → 2 bytes
[6–9]  energia     uint32  × 0.001 kWh → 4 bytes
[10]   fator pot.  uint8   × 0.01    → 1 byte
[11]   frequência  uint8   × 0.1 Hz  → 1 byte
[12–13] reservado                    → 2 bytes
```

Formatos recomendados: **Cayenne LPP**, **Protocol Buffers**, **MessagePack**.

**Opção C — Usar downlinks apenas para comandos curtos:**

LoRaWAN não foi projetado para transferência de dados grandes. Use o downlink exclusivamente para comandos de controle curtos (ex: mudar intervalo de transmissão, ligar/desligar relé, solicitar leitura imediata) — que tipicamente cabem em 1 a 4 bytes — e trafegue dados maiores por canal alternativo (MQTT, HTTP) quando disponível.
...
### ❌ Let's Encrypt — `429 rateLimited`: limite de 5 certificados por semana atingido

Este é um dos erros mais frustrantes no ciclo de desenvolvimento com cert-manager, pois **bloqueia completamente a emissão de novos certificados por até 7 dias** para um domínio específico. Ocorre quando o mesmo conjunto exato de nomes de domínio (identifiers) é usado para solicitar certificados repetidamente — algo comum durante reinstalações de cluster, ajustes de Ingress, ou deleção acidental de Secrets TLS.

#### Como se manifesta

O certificado fica com `READY = False` indefinidamente. Ao descrever o objeto `Certificate`:

```bash
kubectl describe certificate <nome-do-certificado> -n <namespace>
```

A saída mostra um evento de falha como este:

```
Warning  Failed  cert-manager-certificates-issuing
  The certificate request has failed to complete and will be retried:
  Failed to wait for order resource "<nome>-<hash>" to become ready:
  order is in "errored" state: Failed to create Order: 429
  urn:ietf:params:acme:error:rateLimited: too many certificates (5) already
  issued for this exact set of identifiers in the last 168h0m0s,
  retry after <data e hora UTC>:
  see https://letsencrypt.org/docs/rate-limits/#new-certificates-per-exact-set-of-identifiers
```

O campo-chave é `retry after <data e hora UTC>` — é exatamente quando o bloqueio expira.

#### Causa raiz

O Let's Encrypt impõe um limite de **5 certificados por semana (168 horas) para um conjunto exato de domínios**. Cada vez que o cert-manager solicita um certificado — seja por reinstalação do cluster, deleção manual do Secret TLS, reaplicação do Ingress, ou qualquer motivo que faça o objeto `Certificate` ser recriado — uma nova emissão é contabilizada. Ao atingir 5, a API retorna HTTP 429 e rejeita todas as tentativas subsequentes até a janela de 7 dias zerar.

> ⚠️ O limite é por **conjunto exato de identifiers** (nomes de domínio). `chirpstack-v3.seudominio.com.br` e `chirpstack-v3.seudominio.com.br` + `chirpstack-v4.seudominio.com.br` são contados separadamente — mas a mesma combinação repetida 5 vezes ativa o bloqueio. Em clusters com vários subdomínios (como este projeto, que tem `chirpstack-v3`, `chirpstack-v4`, `chirpstack-v4-rest-api` etc.), o risco aumenta significativamente durante reinstalações.

#### Diagnóstico completo

```bash
# 1. Ver o status de todos os certificados do namespace
kubectl get certificate -A
# Procure por READY = False

# 2. Descrever o certificado afetado para ver a mensagem de rate limit
kubectl describe certificate <nome-do-cert> -n <namespace>

# 3. Ver os CertificateRequests (tentativas de emissão)
kubectl get certificaterequest -n <namespace>
kubectl describe certificaterequest <nome-do-cr> -n <namespace>

# 4. Ver os Orders (pedidos à API ACME do Let's Encrypt)
kubectl get order -n <namespace>
kubectl describe order <nome-do-order> -n <namespace>
# Procure por: order is in "errored" state

# 5. Ver os Challenges (se existirem — geralmente não chegam a ser criados quando há rate limit)
kubectl get challenge -n <namespace>

# 6. Confirmar que o Secret TLS não existe (causa que disparou a reemissão)
kubectl get secret -n <namespace> | grep tls
```

**Saída esperada que confirma o cenário** (exemplo com dois namespaces, um afetado e outro não):

```
NAMESPACE        NAME                       READY   SECRET                     AGE
chirpstack-v3    chirpstack-v3-tls          True    chirpstack-v3-tls          2d
chirpstack-v4    chirpstack-v4-tls          False   chirpstack-v4-tls          30m
```

Neste exemplo, o certificado do `chirpstack-v3` foi emitido com sucesso antes de atingir o limite. O do `chirpstack-v4` falhou porque o limite já havia sido atingido naquela janela de 7 dias — situação comum quando múltiplos subdomínios do mesmo projeto são registrados em sequência durante reinstalações ou ajustes de manifesto.

#### O que fazer enquanto aguarda o desbloqueio

O bloqueio é automático — não há como solicitar remoção. A única saída limpa é esperar. Enquanto isso, há três abordagens:

---

**Opção A — Usar `letsencrypt-staging` para validar o fluxo (recomendada)**

O ambiente de staging do Let's Encrypt tem limites muito mais altos (não bloqueia em situações normais de desenvolvimento) e permite validar toda a cadeia cert-manager → Ingress → ACME → HTTPS **sem consumir quota do ambiente de produção**. O certificado emitido não é confiável pelos browsers, mas o serviço responde via HTTPS e os testes funcionam normalmente.

Este cluster já inclui o `ClusterIssuer` de staging por padrão (criado junto com a Homepage). Se ele não existir no seu ambiente, crie-o:
...
Em seguida, troque a anotação do Ingress afetado para apontar para o staging:

```bash
kubectl annotate ingress <nome-do-ingress> \
  -n <namespace> \
...
Com isso o Ingress volta a funcionar via HTTPS (com aviso de certificado não confiável no browser) e você pode continuar o desenvolvimento e os testes de integração com o ChirpStack.

---

**Opção B — Aguardar e reemitir após o desbloqueio**

A mensagem de erro indica exatamente quando o bloqueio expira:

```
retry after <data e hora UTC — indicada na mensagem de erro>
```

Após esse horário, delete o objeto `Certificate` para forçar uma nova tentativa de emissão:

```bash
# Deletar o Certificate faz o cert-manager recriar tudo do zero
kubectl delete certificate <nome-do-cert> -n <namespace>

# Certificar que o Ingress ainda tem a anotação correta apontando para prod
kubectl describe ingress <nome-do-ingress> -n <namespace> | grep cluster-issuer

# Reaplicar o Ingress se necessário
...
**Opção C — Certificado wildcard com DNS-01 (solução definitiva para múltiplos subdomínios)**

Este projeto expõe vários subdomínios sob o mesmo domínio raiz (`chirpstack-v3`, `chirpstack-v4`, `chirpstack-v4-rest-api`, `k8s` etc.). A melhor estratégia de longo prazo é emitir **um único certificado wildcard** `*.seudominio.com.br` que cobre todos eles com uma única emissão — eliminando completamente o risco de atingir o rate limit por subdomínio.

Requer validação DNS-01 (não HTTP-01), que exige integração com o provedor de DNS. Exemplo de manifesto:

```yaml
apiVersion: cert-manager.io/v1
kind: Certificate
metadata:
  name: wildcard-seudominio
  namespace: <namespace>
spec:
  secretName: wildcard-seudominio-tls
  issuerRef:
...
Consulte a documentação do cert-manager para configurar o solver DNS-01 com seu provedor: https://cert-manager.io/docs/configuration/acme/dns01/

---

#### Anotações conflitantes no Ingress que agravam o problema

Um ponto que pode contribuir para o acúmulo de tentativas de emissão é a presença de **anotações duplicadas com valores conflitantes** no manifesto do Ingress. O Kubernetes aplica a última ocorrência de uma chave duplicada no YAML, mas o comportamento pode ser inconsistente dependendo da versão da ferramenta que está aplicando o manifesto, podendo forçar reconciliações desnecessárias.

Exemplo de conflito comum em manifestos copiados ou gerados incrementalmente:

```yaml
# ❌ Conflito — mesma chave com valores opostos
nginx.ingress.kubernetes.io/ssl-redirect: "false"   # linha 1 — ignorada
# ... outras anotações ...
nginx.ingress.kubernetes.io/ssl-redirect: "true"    # linha 2 — vence, mas causa confusão
```

**Corrija removendo a duplicata e mantenha apenas:**
...
Além disso, o `pathType: Prefix` **não processa expressões regulares** — ele usa correspondência de prefixo literal. Para usar padrões como `/?(.*)` no path com rewrite, o tipo correto é `ImplementationSpecific`:

```yaml
# ❌ Errado — ignora o regex, comportamento inesperado com rewrite-target
- path: /?(.*)
  pathType: Prefix

# ✅ Correto — processa o regex via Ingress NGINX
- path: /?(.*)
  pathType: ImplementationSpecific
```

> ⚠️ Quando um Ingress concentra múltiplos serviços no mesmo host, sempre coloque paths mais específicos (como `/api/?(.*)`) **antes** do path coringa (`/?(.*)`) — o Ingress NGINX avalia as regras na ordem em que aparecem no manifesto e a primeira correspondência vence.

#### Resumo do cenário e resolução

| Elemento | Status | Detalhe |
|---|---|---|
| Certificado do serviço A | ✅ `READY=True` | Emitido com sucesso antes de atingir o limite |
| Certificado do serviço B | ❌ `READY=False` | Rate limit atingido — 5 emissões em 7 dias |
| Secret TLS do serviço B | ❌ Não existe | Ausente ou deletado, disparou nova tentativa de emissão |
| Desbloqueio | ⏳ Automático | Após `retry after` indicado na mensagem de erro |
| Ação imediata | 🔧 Usar staging | Trocar `cluster-issuer` para `letsencrypt-staging` |
| Ação pós-desbloqueio | 🔁 Deletar Certificate | `kubectl delete certificate <nome> -n <namespace>` e reaplicar |

> 💡 **Boas práticas para evitar o rate limit:** nunca delete o Secret TLS manualmente sem necessidade; use `letsencrypt-staging` durante o desenvolvimento e testes de Ingress; consolide subdomínios em um certificado wildcard se o projeto crescer; monitore o status dos certificados com `kubectl get certificate -A` antes de reinstalar ou recriar recursos.
...
O workflow falha imediatamente no step de `Checkout` com esse erro quando o repositório é **privado** e o step não recebe um token de autenticação.

O `actions/checkout@v5` exige token explícito para repositórios privados — o `GITHUB_TOKEN` padrão não é suficiente nesse contexto. Todos os jobs que fazem checkout precisam do `GH_PAT`, incluindo o job `detect` (que normalmente é esquecido por ser apenas de detecção de mudanças).

**Solução:** certifique-se de que **todos** os steps de checkout no workflow têm o token configurado:
...
> ⚠️ O `fetch-depth: 0` é obrigatório junto com o token nos jobs que usam `dorny/paths-filter` ou que criam tags Git — sem ele o histórico de commits fica truncado e a detecção de paths alterados falha.

Verifique também que o secret `GH_PAT` está cadastrado no repositório em **Settings → Secrets and variables → Actions → Repository secrets** — criá-lo apenas em **Developer settings → Personal access tokens** não é suficiente, pois o runner não tem acesso automático a tokens pessoais.

---

### ❌ GitHub Actions — `refusing to allow a Personal Access Token to create or update workflow`

O pipeline falha no step **Criar tag Git** com o erro:

```
! [remote rejected] <tag> -> <tag> (refusing to allow a Personal Access Token
to create or update workflow `.github/workflows/main.yml` without `workflow` scope)
```

Isso acontece porque o repositório contém arquivos em `.github/workflows/` e o GitHub bloqueia qualquer push de tag que referencie commits com arquivos de workflow quando o PAT não tem o scope `workflow` — mesmo que a intenção seja apenas criar uma tag de versão, não modificar o workflow.

**Solução:** adicionar o scope `workflow` ao token existente:

1. Acesse **GitHub → Settings → Developer settings → Personal access tokens → Tokens (classic)**
2. Clique no token `GH_PAT` → **Edit**
3. Marque ✅ `workflow` (além do `repo` que já deve estar marcado)
4. Clique em **Regenerate token** e **copie o novo valor** (exibido apenas uma vez)
5. Atualize o secret no repositório: **Settings → Secrets and variables → Actions → `GH_PAT` → Update**

> O PAT precisa ter obrigatoriamente os scopes `repo` **e** `workflow` para funcionar corretamente neste pipeline.

---

### ❌ `409-NAMESPACE_CONFLICT` ao criar repositório no OCI Container Registry via Terraform

O `tofu apply` falha após ~2 minutos com o erro abaixo ao tentar criar repositórios no OCI Container Registry:

```
Error: 409-NAMESPACE_CONFLICT, Repository already exists.
```

**Causa:** o OCI Container Registry agrupa repositórios por namespace baseado no prefixo do nome. Quando dois repositórios compartilham o mesmo prefixo — como `cslabweb-backend_platform_linux-arm64` e `cslabweb-frontend_platform_linux-arm64` — o OCI os trata como pertencentes ao mesmo namespace e rejeita a criação do segundo com `NAMESPACE_CONFLICT`, mesmo que o repositório não exista ainda. O Terraform não salva o recurso no state porque a API retornou erro, mas a OCI pode ter criado o repositório parcialmente.

**Solução — usar namespace hierárquico com barra `/`:**

O padrão correto do OCI Registry para separar repositórios com prefixo comum é usar `/` como separador de namespace no `display_name`:

```hcl
# ❌ Errado — causa NAMESPACE_CONFLICT quando há dois repositórios com mesmo prefixo
display_name = "cslabweb-backend_platform_linux-arm64"
display_name = "cslabweb-frontend_platform_linux-arm64"

# ✅ Correto — namespace hierárquico, sem conflito
display_name = "cslabweb/backend_platform_linux-arm64"
display_name = "cslabweb/frontend_platform_linux-arm64"
```
...
**1.** Delete os repositórios criados parcialmente na OCI (se existirem):

```bash
# Listar repositórios com o prefixo afetado
oci artifacts container repository list \
  --compartment-id <SEU_COMPARTMENT_OCID> \
  --all \
  --query 'data.items[*].{"nome":"display-name","ocid":id}' \
  --output table

# Deletar cada um que precisar ser recriado com o nome correto
oci artifacts container repository delete \
  --repository-id <OCID_DO_REPOSITORIO> \
  --force
```

**2.** Atualize o `display_name` no arquivo `.tf` para usar `/`:

```hcl
resource "oci_artifacts_container_repository" "cslabweb-backend_platform_linux-arm64" {
...
resource "oci_artifacts_container_repository" "cslabweb-frontend_platform_linux-arm64" {
  compartment_id = var.compartment_id
  display_name   = "cslabweb/frontend_platform_linux-arm64"
  is_immutable   = false
  is_public      = false
...
**3.** Atualize as referências no `main.yml` do workflow:

```yaml
# Backend
-t ${{ secrets.OCI_REGISTRY_URL }}/${{ secrets.OCI_REGISTRY_OBJECT_STORAGE_NAMESPACE }}/cslabweb/backend_platform_linux-arm64:${{ steps.version.outputs.VERSION }}
-t ${{ secrets.OCI_REGISTRY_URL }}/${{ secrets.OCI_REGISTRY_OBJECT_STORAGE_NAMESPACE }}/cslabweb/backend_platform_linux-arm64:latest

# Frontend
-t ${{ secrets.OCI_REGISTRY_URL }}/${{ secrets.OCI_REGISTRY_OBJECT_STORAGE_NAMESPACE }}/cslabweb/frontend_platform_linux-arm64:${{ steps.version.outputs.VERSION }}
-t ${{ secrets.OCI_REGISTRY_URL }}/${{ secrets.OCI_REGISTRY_OBJECT_STORAGE_NAMESPACE }}/cslabweb/frontend_platform_linux-arm64:latest
```

**4.** Se outros repositórios já existem na OCI mas não estão no state do Terraform (criados em runs anteriores com falha), importe-os antes do próximo apply:

```bash
# Gerar automaticamente todos os comandos de import
oci artifacts container repository list \
  --compartment-id <SEU_COMPARTMENT_OCID> \
  --all \
  --query 'data.items[*].{"nome":"display-name","ocid":id}' \
  --raw-output | \
jq -r '.[] | "tofu import \x27module.oci-infra_ci_cd.oci_artifacts_container_repository.\(.nome)\x27 \(.ocid)"'
```

**5.** Rode o apply com paralelismo 1 para evitar race conditions na API do OCI:

```bash
tofu apply -parallelism=1
```

> 💡 **Regra geral:** sempre que dois ou more repositórios no OCI Container Registry compartilharem o mesmo prefixo antes do primeiro hífen, use o padrão `prefixo/nome_repositorio` com barra no `display_name` para evitar conflitos de namespace.

---


### ❌ Docker build — `npm error Missing: <pacote>@<versão> from lock file`

O build da imagem Docker falha no passo `RUN npm ci` com uma mensagem do tipo:

```
npm error `npm ci` can only install packages when your package.json and
package-lock.json are in sync. Please update your lock file before continuing.
npm error Missing: zod@3.25.76 from lock file
```

**Causa raiz:**

O `npm ci` é estritamente síncrono — ele exige que `package.json` e `package-lock.json` estejam 100% em sincronia. Situações comuns que provocam esse descompasso:

- O `package.json` foi editado manualmente (nova dependência, atualização de versão) sem executar `npm install` em seguida para regenerar o lock file.
- O `package-lock.json` foi ignorado no `.gitignore` e não foi versionado — o CI/CD recebe apenas o `package.json`.
- Um merge de branches gerou conflito no lock file que foi resolvido incorretamente (ex: aceitar apenas um lado do conflito sem reger o `npm install`).
- Uma dependência indireta (`peerDependency`) foi atualizada em outro pacote e exige uma versão do lock que ainda não está presente.

> No caso específico deste projeto, o pacote `primeclt` puxa internamente o `openai` SDK, que declara `zod ^3.23.8` como peer dependency. Como o lock file resolve `zod` para a versão `4.x`, pode surgir conflito de peer deps dependendo da versão do npm usada no builder.

**Passo 1 — Verificar se `package-lock.json` está no `.gitignore`:**

```bash
# Executar na raiz do projeto de frontend
cat .gitignore | grep package-lock
```

Se aparecer listado, **remova essa linha** — o lock file deve sempre ser versionado em projetos com CI/CD. O `.gitignore` do projeto `05.chirpstack_consumer/app_chirpstack_consumer_frontend/` não bloqueia o lock file ✅.

**Passo 2 — Sincronizar o lock file localmente:**

```bash
cd 05.chirpstack_consumer/app_chirpstack_consumer_frontend
npm install
```

O `npm install` regenera o `package-lock.json` resolvendo todas as dependências com base no `package.json` atual.

**Passo 3 — Resolver conflitos de peer deps com `overrides` (se necessário):**

Se o erro persistir por causa de um conflito de versão entre dependências indiretas (como `primeclt → openai → zod@^3.x` vs `zod@4.x` resolvido no lock), adicione um bloco `"overrides"` no final do `package.json` para forçar uma versão única:
...
O bloco `"overrides"` deve ser o último campo do objeto JSON, após `"devDependencies"`.

**Passo 4 — Commitar e fazer push do lock file atualizado:**

```bash
git add package.json package-lock.json
git commit -m "chore: sync package-lock.json and add zod override"
git push
```

O `npm ci` no Dockerfile voltará a funcionar normalmente porque o lock file agora está sincronizado com o `package.json`.

**Verificação extra — confirmar a versão do Node no build:**

O Dockerfile usa `node:22-alpine`. Certifique-se de que a versão do Node local é compatível para evitar diferenças no lock file gerado:
...
Se houver diferença de versão entre o ambiente local e o builder Docker, use o mesmo container para regenerar o lock:
...
### ❌ ABP — "Frame-counter reset or rollover detected" após reset do dispositivo

**Sintoma:**

Após um reset (reboot, corte de energia ou regravação do firmware), o dispositivo ABP volta a transmitir normalmente — o gateway recebe os frames — mas o ChirpStack rejeita todos os uplinks e registra nos Events do device:

```
Frame-counter reset or rollover detected
```

O dispositivo precisa ser resetado diversas vezes ou o frame counter no ChirpStack precisa ser zerado manualmente para voltar a funcionar.

**Causa raiz — o que é o frame counter e por que ele reseta:**

Em LoRaWAN, cada uplink ABP carrega um **frame counter** (`FCnt`, também chamado `seqnoUp`) que incrementa a cada transmissão. O ChirpStack armazena o último valor aceito e rejeita qualquer frame cujo contador seja igual ou menor — isso previne ataques de replay, onde um agente malicioso captura e reenvia frames antigos.

O problema é que o LMiC mantém o `seqnoUp` apenas na SRAM do microcontrolador. A SRAM é **volátil** — ao resetar o dispositivo, o contador volta a zero. Do ponto de vista do ChirpStack, receber um frame com `FCnt = 0` quando o último aceito era, por exemplo, `FCnt = 47` parece um ataque de replay, então o frame é descartado.

```
Sequência do problema:

  Boot 1:  FCnt = 0, 1, 2, ... 47   → ChirpStack aceita (último = 47)
  Reset
  Boot 2:  FCnt = 0                 → ChirpStack rejeita ("Frame-counter reset detected")
           FCnt = 1                 → ChirpStack rejeita
           FCnt = 2                 → ChirpStack rejeita
           ...todos os frames são descartados indefinidamente
```

**O que é o NVS e por que ele resolve:**

O **NVS** (*Non-Volatile Storage*) é o sistema de armazenamento persistente do ESP32. Diferentemente da SRAM, dados gravados no NVS **sobrevivem a reboots, resets e cortes de energia** — ficam armazenados em uma partição dedicada da flash interna do ESP32 (24 KB por padrão na tabela de partições `default`).

Os dados são organizados em **namespaces** e **chaves**, funcionando como um dicionário:
...
O acesso é feito via `Preferences.h`, biblioteca nativa do ESP32:

```cpp
Preferences prefs;
prefs.begin("lmic", false);               // abre namespace em leitura/escrita
prefs.putUInt("seqnoUp", LMIC.seqnoUp);  // grava o contador
uint32_t v = prefs.getUInt("seqnoUp", 0); // lê (0 = padrão no primeiro boot)
```

Comparativo das memórias do ESP32:

| Memória | Persistente | Tamanho típico | Uso |
|---|---|---|---|
| SRAM | ❌ | 520 KB | Variáveis em tempo de execução |
| Flash (programa) | ✅ | 4 MB | Firmware gravado pelo IDE |
| **NVS (flash)** | ✅ | **24 KB** | **Configurações, contadores, credenciais** |
| EEPROM (emulada) | ✅ | até 4 KB | API legada, usa NVS internamente |

> No Arduino Nano (ATmega328P) o equivalente é a **EEPROM real** de 1 KB, acessada via `EEPROM.h` — lá não existe NVS.

**Opção A — desenvolvimento (não recomendado para produção):**

Desabilitar a validação do frame counter no ChirpStack:

> Devices → seu device → **Configuration** → ✅ **Disable frame-counter validation**

Simples, mas nunca deve ser usado em produção pois remove a proteção contra replay attacks.

**Opção B — produção (recomendada): persistência em NVS:**

Salvar o `seqnoUp` no NVS após cada transmissão e restaurá-lo no boot. Com isso o contador nunca regride — após um reset o dispositivo retoma exatamente do próximo valor esperado pelo ChirpStack.

```
Sequência com NVS:

  Boot 1:  FCnt = 0, 1, 2, ... 47   → ChirpStack aceita; NVS salva 48
  Reset
  Boot 2:  NVS restaura seqnoUp = 48
           FCnt = 48                 → ChirpStack aceita ✅
           FCnt = 49                 → ChirpStack aceita ✅
```

Implementação no firmware (ESP32 + MCCI LMiC):
...
// Salva o contador após cada TX — chamar dentro de EV_TXCOMPLETE
static void saveSeqnoUp(void) {
    prefs.putUInt("seqnoUp", LMIC.seqnoUp);
}

// Restaura o contador no boot — chamar após LMIC_setSession()
static uint32_t loadSeqnoUp(void) {
    return prefs.getUInt("seqnoUp", 0);
}
```

Em `setup()`, **após** `LMIC_setSession()`:

```cpp
// Abre o namespace NVS em leitura/escrita
prefs.begin("lmic", false);

os_init();
LMIC_reset();

LMIC_setSession(0x000000, DEVADDR, (u1_t*)NWKSKEY, (u1_t*)APPSKEY);

// IMPORTANTE: deve vir APÓS LMIC_setSession() pois setSession()
// reseta seqnoUp para 0 internamente — restaurar aqui sobrescreve isso.
// No primeiro boot, loadSeqnoUp() retorna 0 (valor padrão correto).
LMIC.seqnoUp = loadSeqnoUp();
```

Em `onEvent()`, dentro de `EV_TXCOMPLETE`:

```cpp
case EV_TXCOMPLETE:
    // O LMiC já incrementou seqnoUp antes de disparar EV_TXCOMPLETE,
    // então o valor salvo aqui é sempre o próximo a ser usado —
    // garantindo sequência estritamente crescente mesmo após reset abrupto.
    saveSeqnoUp();
...
**Ponto crítico — ordem das chamadas no `setup()`:**

```cpp
// ✅ CORRETO — restaurar APÓS setSession()
LMIC_setSession(...);
LMIC.seqnoUp = loadSeqnoUp();  // sobrescreve o 0 que setSession() colocou

// ❌ ERRADO — restaurar ANTES de setSession()
LMIC.seqnoUp = loadSeqnoUp();
LMIC_setSession(...);           // apaga o valor restaurado, volta para 0
```

**Verificação no Serial Monitor:**

Após implementar, o Serial Monitor deve mostrar na inicialização:
...
ABP session provisioned:
  ...
  NwkSKey : 15-1F-77-79-...
  AppSKey : FD-91-D5-5D-...
  seqnoUp : 48          ← contador restaurado do NVS após reset
```

E após cada transmissão:

```
EV_TXCOMPLETE (includes waiting for RX windows)
  seqnoUp saved to NVS: 49
```

O evento `Frame-counter reset or rollover detected` não deve mais aparecer no ChirpStack.

---

### ❌ Uplinks chegam ao gateway mas nunca aparecem nos Events do device (MIC inválido — MCCI LMiC v6)

**Sintoma:**

O gateway recebe os frames do dispositivo normalmente — eles aparecem em **Gateways → Live LoRaWAN Frames** — mas a aba **Devices → Events** permanece completamente vazia, e o log do pod ChirpStack repete continuamente:

```
INFO up{...}:data_up: chirpstack::uplink::data:
None of the device-sessions for dev_addr resulted in valid MIC
dev_addr=00aa88ca
```

O frame chega com `CRC_OK`, RSSI excelente e frequência correta, mas o ChirpStack descarta o uplink após verificar o MIC.

**Causa raiz:**

A biblioteca **MCCI LoRaWAN LMIC v6.x** introduziu suporte interno a LoRaWAN 1.1, que exige múltiplas chaves de sessão derivadas (`FNwkSIntKey`, `SNwkSIntKey`, `NwkSEncKey`). Em dispositivos ABP configurados para LoRaWAN 1.0.x, a v6 deriva chaves adicionais a partir da `NwkSKey` e as utiliza internamente para criptografia e cálculo do MIC — **em vez de usar a `NwkSKey` diretamente como especificado pelo LoRaWAN 1.0**.

Isso faz com que o MIC calculado pelo dispositivo nunca bata com o MIC esperado pelo ChirpStack, mesmo que todas as credenciais estejam corretas nos dois lados. O problema é silencioso e extremamente difícil de diagnosticar porque:

- As chaves impressas no Serial Monitor estão corretas.
- `LMIC.nwkKey` e `LMIC.artKey` contêm os valores corretos.
- O frame é transmitido normalmente pelo rádio.
- O ChirpStack encontra o `dev_addr` no banco mas rejeita o MIC silenciosamente.

**O que é o MIC e como ele é calculado:**

O **MIC** (*Message Integrity Code*) é um código de autenticação de 4 bytes anexado ao final de cada frame LoRaWAN. Ele garante que o frame não foi adulterado em trânsito e que o remetente conhece a `NwkSKey`. O ChirpStack recalcula o MIC ao receber o frame e compara com o MIC transmitido — se diferirem, o frame é descartado silenciosamente.

O cálculo usa **AES-128-CMAC** (cipher-based MAC), definido pelo padrão LoRaWAN 1.0:

```
MIC = AES-128-CMAC(NwkSKey, B0 | msg)[0:4]
```

Onde `B0` é um bloco de 16 bytes montado com metadados do frame:

```
B0 = 0x49 | 0x00 0x00 0x00 0x00 | Dir | DevAddr(LE,4) | FCnt(LE,4) | 0x00 | len(msg)
```

E `msg` é o frame completo sem o MIC:

```
msg = MHDR | DevAddr(LE,4) | FCtrl | FCnt(LE,2) | FPort | FRMPayload
```

**Exemplo concreto** com os dados deste projeto (DevAddr `00aa88ca`, FCnt=0):

```
Frame transmitido (hex):
40 CA 88 AA 00  00 00  00  01  [14 bytes payload]  [4 bytes MIC]
│  └─ DevAddr LE ─┘  │   │  └─ FPort              └─ MIC
└─ MHDR(0x40)        │   └─ FCnt LE
                     └─ FCtrl

B0 construído:
49 00 00 00 00 00   ← flags + zeros
CA 88 AA 00         ← DevAddr little-endian
00 00 00 00         ← FCnt 32-bit little-endian
00                  ← zero
17                  ← len(msg) = 23 bytes

Cálculo:
MIC = AES-128-CMAC(
        key  = 151f7779d7aaee422a74568380db7045,
        data = B0 || msg
      )[0:4]
    = 14 2e dc be  ✅  (bate com o que o ChirpStack esperava)
```

Com a MCCI LMiC v6, o MIC calculado pelo dispositivo era sempre `2c a4 ec e8` — um valor que não corresponde a nenhuma combinação conhecida de chaves — porque a biblioteca usava uma chave derivada internamente em vez da `NwkSKey` original.

**Como diagnosticar o MIC manualmente:**
...
# Exemplo com os dados deste projeto
mic = calcular_mic(
    nwkskey_hex       = "151f7779d7aaee422a74568380db7045",
    devaddr_hex       = "00aa88ca",   # big-endian, como no ChirpStack
    fcnt              = 0,
    frame_sem_mic_hex = "40CA88AA0000000001F56CB7FFFA9C7C0D6C6C72368ECD"
)
print(f"MIC calculado: {mic}")   # → 142edcbe
```
...
public class LoRaWANMic {
    ...
    /**
     * Calcula o MIC de um frame LoRaWAN 1.0 (uplink).
     *
     * @param nwksKeyHex      NwkSKey em hex (32 chars, big-endian como no ChirpStack)
     * @param devAddrHex      DevAddr em hex (8 chars, big-endian como no ChirpStack)
     * @param fcnt            Frame counter (uplink)
     * @param frameWithoutMic Frame completo sem os 4 bytes finais de MIC (hex)
     * @return MIC calculado em hex (8 chars, ex: "142edcbe")
     */
    public static String calcularMic(String nwksKeyHex,
        ...
        byte[] devAddrLe = new byte[]{
            devAddrBe[3], devAddrBe[2], devAddrBe[1], devAddrBe[0]
        };
        ...
        // Retorna apenas os primeiros 4 bytes
        return hex.formatHex(mic, 0, 4);
    }

    /**
     * AES-128-CMAC conforme RFC 4493.
     * Usa apenas javax.crypto — sem dependências externas.
     */
    ...
    public static void main(String[] args) throws Exception {
        ...
        String mic = calcularMic(
            "151f7779d7aaee422a74568380db7045",  // NwkSKey (big-endian, como no ChirpStack)
            "00aa88ca",                           // DevAddr (big-endian, como no ChirpStack)
            0,                                    // FCnt
            "40CA88AA0000000001F56CB7FFFA9C7C0D6C6C72368ECD"
        );
        System.out.println("MIC calculado: " + mic);  // → 142edcbe
    }
}
```
...
Se o MIC calculado por qualquer um dos scripts **não bater** com o MIC no frame capturado no gateway, as chaves no dispositivo diferem das cadastradas no ChirpStack — mesmo que o Serial Monitor mostre os valores corretos.

**Solução:**

Fazer downgrade da biblioteca para a versão **3.3.0**, que implementa LoRaWAN 1.0.x corretamente para ABP:

```bash
# Remover a v6 incompatível
rm -rf ~/Apps/Arduino/libraries/MCCI_LoRaWAN_LMIC_library

# Instalar a v3.3.0 via git
cd ~/Apps/Arduino/libraries
git clone \
  https://github.com/AdailSilva/IBM-LMIC-LoRaWAN-MAC-in-C-library-v3.3.0.git \
  MCCI_LoRaWAN_LMIC_library
```

> **Atenção:** A v3.3.0 conflita com o ESP32 Arduino core 3.x por causa de uma definição duplicada de `hal_init`. Se ocorrer o erro `multiple definition of 'hal_init'`, renomeie a função na biblioteca:
>
> ```bash
> sed -i 's/void hal_init(/void lmic_hal_init(/g' \
>   ~/Apps/Arduino/libraries/MCCI_LoRaWAN_LMIC_library/src/hal/hal.cpp
>
> sed -i 's/hal_init()/lmic_hal_init()/g' \
>   ~/Apps/Arduino/libraries/MCCI_LoRaWAN_LMIC_library/src/lmic/lmic.c \
>   ~/Apps/Arduino/libraries/MCCI_LoRaWAN_LMIC_library/src/hal/hal.cpp
> ```

**Verificação:**

Após o downgrade, compile e grave o firmware. Os uplinks aparecerão normalmente na aba **Events** do device:

```
INFO up{...}:data_up: chirpstack::storage::device_gateway:
Gateway rx-info saved dev_eui=08fdbe4bc604e8f8
```

**Versões afetadas:**

| Componente | Versão com problema | Versão correta |
|---|---|---|
| MCCI LoRaWAN LMIC library | [**6.0.1**](https://github.com/AdailSilva/IBM-LMIC-LoRaWAN-MAC-in-C-library-v6.0.1) (e possivelmente 4.x, 5.x) | [**3.3.0**](https://github.com/AdailSilva/IBM-LMIC-LoRaWAN-MAC-in-C-library-v3.3.0) |
| ESP32 Arduino core | 3.3.8 | 3.3.8 (compatível após patch `hal_init`) |
| ChirpStack | v4 | v4 (sem alteração necessária) |
| Activation | ABP | ABP |
| MAC version | LoRaWAN 1.0.3 | LoRaWAN 1.0.3 |
