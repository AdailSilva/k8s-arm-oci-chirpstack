...
docker buildx build \
  --platform linux/arm64 \
  -t gru.ocir.io/<OCI_REGISTRY_OBJECT_STORAGE_NAMESPACE>/api-k8s-dashboard-backend_platform_linux-arm64:latest \
  --no-cache --push \
...
docker buildx build \
  --platform linux/arm64 \
  -t gru.ocir.io/<OCI_REGISTRY_OBJECT_STORAGE_NAMESPACE>/app-k8s-dashboard-frontend_platform_linux-arm64:latest \
  --no-cache --push \
...
kubectl get svc     api-k8s-dashboard-backend-service -n k8s-dashboard
kubectl get endpoints api-k8s-dashboard-backend-service -n k8s-dashboard
