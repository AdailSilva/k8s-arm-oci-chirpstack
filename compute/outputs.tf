output "leader" {
  value = {
    ...
    hostname   = oci_core_instance.leader.display_name
    #hostname   = oci_core_instance.leader.hostname_label
    fqdn       = local.leader_fqdn
    private_ip = oci_core_instance.leader.private_ip
    ssh_lb = {
      listener_id = oci_network_load_balancer_listener.leader_tcp_22.id
      backend_id  = oci_network_load_balancer_backend.leader_tcp_22.id
    }
  }
}
